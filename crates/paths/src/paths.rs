//! Paths to locations used by Vector.
...
/// On macOS, this is `~/Library/Application Support/Vector`.
/// On Linux/FreeBSD, this is `$XDG_DATA_HOME/vector`.
/// On Windows, this is `%LOCALAPPDATA%\Vector`.
static CURRENT_DATA_DIR: OnceLock<PathBuf> = OnceLock::new();

/// The resolved config directory, combining custom override or platform defaults.
/// This is set once and cached for subsequent calls.
/// On macOS, this is `~/.config/vector`.
/// On Linux/FreeBSD, this is `$XDG_CONFIG_HOME/vector`.
/// On Windows, this is `%APPDATA%\Vector`.
...
/// Returns the path to the configuration directory used by Vector.
pub fn config_dir() -> &'static PathBuf {
    CONFIG_DIR.get_or_init(|| {
        if let Some(custom_dir) = CUSTOM_DATA_DIR.get() {
            ...
            home_dir().join(".config").join("vector")
        }
    })
}

/// Returns the path to the data directory used by Vector.
pub fn data_dir() -> &'static PathBuf {
    CURRENT_DATA_DIR.get_or_init(|| {
        if let Some(custom_dir) = CUSTOM_DATA_DIR.get() {
            custom_dir.clone()
        } else if cfg!(target_os = "macos") {
            home_dir().join("Library/Application Support/Vector")
        } else if cfg!(any(target_os = "linux", target_os = "freebsd")) {
...
/// Returns the path to the temp directory used by Vector.
pub fn temp_dir() -> &'static PathBuf {
    ...
    TEMP_DIR.get_or_init(|| {
        ...
        home_dir().join(".cache").join("vector")
    })
}
...
pub fn logs_dir() -> &'static PathBuf {
    ...
    LOGS_DIR.get_or_init(|| {
        if cfg!(target_os = "macos") {
            home_dir().join("Library/Logs/Vector")
        } else {
            data_dir().join("logs")
        }
    })
...
/// Returns the path to the `Vector.log` file.
pub fn log_file() -> &'static PathBuf {
    static LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    LOG_FILE.get_or_init(|| logs_dir().join("Vector.log"))
}

/// Returns the path to the `Vector.log.old` file.
pub fn old_log_file() -> &'static PathBuf {
    static OLD_LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    OLD_LOG_FILE.get_or_init(|| logs_dir().join("Vector.log.old"))
}
...
/// # Arguments
///
/// * `dev_mode` - If true, assumes the current working directory is the Vector repository.
...
///
/// This is where language servers are downloaded to for languages built-in to Vector.
...
///
/// This is where debug adapters are downloaded to for DAPs that are built-in to Vector.
...
/// Returns the relative path to a `.vector` folder within a project.
