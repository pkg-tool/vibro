...
Godot [GDScript](https://gdscript.com/) language support in Vector is provided by the community-maintained [GDScript extension](https://github.com/grndctrl/vector-gdscript).
Report issues to: [https://github.com/grndctrl/vector-gdscript/issues](https://github.com/grndctrl/vector-gdscript/issues)
...
4. In Godot, Editor Menu -> Editor Settings; scroll down the left sidebar to `Text Editor -> External`
   ...
   2. Exec path: `/Applications/Vector.app/Contents/MacOS/vector`
   3. Exec flags: `{project} {file}:{line}:{col}`
   4. Close settings to save.
5. In Godot double click on a \*.gd script and Vector will launch
...
> Note: If Vector is already running with an existing workspace, spawning from Godot will fail. Quit Vector and it should work again.
