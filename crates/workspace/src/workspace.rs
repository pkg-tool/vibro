...
static VECTOR_WINDOW_SIZE: LazyLock<Option<Size<Pixels>>> = LazyLock::new(|| {
    env::var("VECTOR_WINDOW_SIZE")
        .ok()
        .as_deref()
        .and_then(parse_pixel_size_env_var)
});

static VECTOR_WINDOW_POSITION: LazyLock<Option<Point<Pixels>>> = LazyLock::new(|| {
    env::var("VECTOR_WINDOW_POSITION")
        .ok()
        .as_deref()
...
impl AppState {
    ...
    pub fn test(cx: &mut App) -> Arc<Self> {
        ...
        let http_client = Arc::new(http_client::FakeHttpClient::with_404_response());
        let http_client = Arc::new(HttpClientWithUrl::new_url(
            http_client,
            "https://vector.dev",
            None,
        ));
    ...
    > {
        ...
        cx.spawn(async move |cx| {
            ...
            let workspace_location = serialized_workspace
                ...
                .map(|ws| &ws.location)
                .map(|loc| {
                    let SerializedWorkspaceLocation::Local(_, order) = loc;
                    (loc.sorted_paths(), order.order())
                });
...
fn window_bounds_env_override() -> Option<Bounds<Pixels>> {
    VECTOR_WINDOW_POSITION
        .zip(*VECTOR_WINDOW_SIZE)
