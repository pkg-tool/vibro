...
const VECTOR_THEME_SCHEMA_URL: &str = "https://vector.dev/schema/themes/v0.2.0.json";
...
struct FamilyMetadata {
    ...
    ///
    /// Use this to ensure certain Vector syntax tokens are matched
...
fn main() -> Result<()> {
    ...
    theme.as_object_mut().unwrap().insert(
        "$schema".to_string(),
        serde_json::Value::String(VECTOR_THEME_SCHEMA_URL.to_string()),
    );
