...
impl DapLocator for PythonLocator {
    ...
    fn create_scenario(
        ...
        let valid_program = build_config.command.starts_with("$VECTOR_")
        ...
        let command = if build_config.command
            == VariableName::Custom("PYTHON_ACTIVE_VECTOR_TOOLCHAIN".into()).template_value()
        {
            VariableName::Custom("PYTHON_ACTIVE_VECTOR_TOOLCHAIN_RAW".into()).template_value()
        } else {
