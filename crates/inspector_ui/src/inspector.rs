...
pub fn init(app_state: Arc<AppState>, cx: &mut App) {
    cx.on_action(|_: &vector_actions::dev::ToggleInspector, cx| {
...
fn render_inspector_id(inspector_id: &InspectorElementId, cx: &App) -> Div {
    ...
    let source_location_string = source_location_string
        .strip_prefix(env!("VECTOR_REPO_DIR"))
    ...
    v_flex()
        ...
        .child(
            div()
                ...
                .tooltip(Tooltip::text("Click to open by running Vector CLI"))
                .on_click(move |_, _window, cx| {
                    cx.background_spawn(open_vector_source_location(source_location))
...
async fn open_vector_source_location(
    location: &'static std::panic::Location<'static>,
) -> anyhow::Result<()> {
    let mut path = Path::new(env!("VECTOR_REPO_DIR")).to_path_buf();
    ...
    let output = new_smol_command("vector")
        .arg(&path_arg)
        .output()
        .await
        .with_context(|| format!("running vector to open {path_arg} failed"))?;

    if !output.status.success() {
        Err(anyhow!(
            "running vector to open {path_arg} failed with stderr: {}",
