...
use gpui::{
    ...
    Styled, TitlebarOptions, UpdateGlobal, Window, WindowKind, WindowOptions, actions,
...
use vector_actions::{
    OpenBrowser, OpenDocs, OpenSettings, OpenVectorUrl, Quit,
};
...
pub fn init(cx: &mut App) {
    ...
    // Ensure a titlebar item is installed for each Workspace. We use a transparent native titlebar
    // on macOS; without a workspace titlebar component, the window controls can overlap content.
    title_bar::init(cx);
...
pub fn build_window_options(display_uuid: Option<Uuid>, cx: &mut App) -> WindowOptions {
    ...
    let window_decorations = match std::env::var("VECTOR_WINDOW_DECORATIONS") {
...
fn initialize_file_watcher(window: &mut Window, cx: &mut Context<Workspace>) {
    if let Err(e) = fs::fs_watcher::global(|_| {}) {
        let message = format!(
            ...
            This may be due to system-wide limits on inotify instances. For troubleshooting see: https://vector.dev/docs/linux
            "#},
            e
        );
        ...
        cx.spawn(async move |_, cx| {
            if prompt.await == Ok(0) {
                cx.update(|cx| {
                    cx.open_url("https://vector.dev/docs/linux#could-not-start-inotify");
                    cx.quit();
                })
                .ok();
            }
...
fn initialize_file_watcher(window: &mut Window, cx: &mut Context<Workspace>) {
    if let Err(e) = fs::fs_watcher::global(|_| {}) {
        let message = format!(
            ...
            This may occur on network filesystems and WSL paths. For troubleshooting see: https://vector.dev/docs/windows
            "#},
            e
        );
        let prompt = window.prompt(
        ...
        cx.spawn(async move |_, cx| {
            if prompt.await == Ok(0) {
                cx.update(|cx| {
                    cx.open_url("https://vector.dev/docs/windows");
                    cx.quit()
                })
                .ok();
            }
...
fn show_software_emulation_warning_if_needed(
    ...
    if specs.is_software_emulated && std::env::var("VECTOR_ALLOW_EMULATED_GPU").is_err() {
        let message = format!(
            db::indoc! {r#"
            Vector uses Vulkan for rendering and requires a compatible GPU.

            Currently you are using a software emulated GPU ({}) which
            will result in awful performance.

            For troubleshooting see: https://vector.dev/docs/linux
            Set VECTOR_ALLOW_EMULATED_GPU=1 env var to permanently override.
            "#},
            specs.device_name
        );
        let prompt = window.prompt(
        ...
        cx.spawn(async move |_, cx| {
            if prompt.await == Ok(1) {
                cx.update(|cx| {
                    cx.open_url("https://vector.dev/docs/linux#vector-fails-to-open-windows");
                    cx.quit();
                })
                .ok();
            }
...
fn register_actions(
    ...
    workspace
        ...
        .register_action(|_, action: &OpenVectorUrl, _, cx| {
        ...
        .register_action({
            ...
            move |_, action: &vector_actions::IncreaseUiFontSize, _, cx| {
                if action.persist {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &vector_actions::DecreaseUiFontSize, _, cx| {
                if action.persist {
                    update_settings_file::<ThemeSettings>(fs.clone(), cx, move |settings, cx| {
                        let ui_font_size = ThemeSettings::get_global(cx).ui_font_size(cx) - px(1.0);
                        let _ = settings
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &vector_actions::ResetUiFontSize, _, cx| {
                if action.persist {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &vector_actions::IncreaseBufferFontSize, _, cx| {
                if action.persist {
                    update_settings_file::<ThemeSettings>(fs.clone(), cx, move |settings, cx| {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &vector_actions::DecreaseBufferFontSize, _, cx| {
                if action.persist {
                    update_settings_file::<ThemeSettings>(fs.clone(), cx, move |settings, cx| {
                        let buffer_font_size =
                            ThemeSettings::get_global(cx).buffer_font_size(cx) - px(1.0);
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &vector_actions::ResetBufferFontSize, _, cx| {
                if action.persist {
                    update_settings_file::<ThemeSettings>(fs.clone(), cx, move |settings, _| {
        ...
        .register_action(|_, _: &install_cli::RegisterVectorScheme, window, cx| {
            cx.spawn_in(window, async move |workspace, cx| {
                install_cli::register_vector_scheme(&cx).await?;
                workspace.update_in(cx, |workspace, _, cx| {
                    struct RegisterVectorScheme;

                    workspace.show_toast(
                        Toast::new(
                            NotificationId::unique::<RegisterVectorScheme>(),
                            format!(
                                "vector:// links will now open in {}.",
            ...
            .detach_and_prompt_err(
                "Error registering vector:// scheme",
                window,
                cx,
                |_, _, _| None,
            );
        ...
        .register_action(|workspace, _: &vector_actions::OpenLicenses, window, cx| {
            open_bundled_file(
                workspace,
        ...
        .register_action(
            move |_: &mut Workspace, _: &vector_actions::OpenKeymap, window, cx| {
                open_settings_file(
                    paths::keymap_file(),
        ...
        .register_action(
            move |workspace, _: &vector_actions::OpenDefaultKeymap, window, cx| {
                open_bundled_file(
                    workspace,
...
mod tests {
    ...
    fn test_bundled_settings_and_themes(cx: &mut App) {
        cx.text_system()
            .add_fonts(vec![
                Assets
                    .load("fonts/plex-mono/VectorPlexMono-Regular.ttf")
                    .unwrap()
                    .unwrap(),
                Assets
                    .load("fonts/plex-sans/VectorPlexSans-Regular.ttf")
                    .unwrap()
                    .unwrap(),
            ])
            .unwrap();
    ...
    fn init_test_with_state(
        ...
        cx.update(move |cx| {
            ...
            tasks_ui::init(cx);
            project::debugger::dap_store::DapStore::init(cx);
            debugger_ui::init(cx);
    ...
    async fn test_opening_project_settings_when_excluded(cx: &mut gpui::TestAppContext) {
        ...
        eprintln!("Created project with .vector/settings.json containing UNIQUEVALUE");
        ...
        let original_settings = app_state
            .fs
            .load(Path::new("/root/.vector/settings.json"))
            .await
            .unwrap();
        ...
        cx.update_global::<SettingsStore, _>(|store, cx| {
            store.update_user_settings::<WorktreeSettings>(cx, |worktree_settings| {
                worktree_settings.file_scan_exclusions = Some(vec![".vector".to_string()]);
            });
        });
        ...
        // 5. Critical: Verify .vector is actually excluded from worktree
        let worktree = cx.update(|cx| project.read(cx).worktrees(cx).next().unwrap().clone());

        let has_vector_entry = cx.update(|cx| worktree.read(cx).entry_for_path(".vector").is_some());
