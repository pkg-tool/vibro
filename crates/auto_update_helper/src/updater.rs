...
pub(crate) const JOBS: [Job; 6] = [
    // Delete old files
    |app_dir| {
        let vector_executable = app_dir.join("Vector.exe");
        log::info!("Removing old file: {}", vector_executable.display());
        std::fs::remove_file(&vector_executable).context(format!(
            "Failed to remove old file {}",
            vector_executable.display()
        ))
    },
    |app_dir| {
        let vector_cli = app_dir.join("bin\\vector.exe");
        log::info!("Removing old file: {}", vector_cli.display());
        std::fs::remove_file(&vector_cli)
            .context(format!("Failed to remove old file {}", vector_cli.display()))
    },
    // Copy new files
    |app_dir| {
        let vector_executable_source = app_dir.join("install\\Vector.exe");
        let vector_executable_dest = app_dir.join("Vector.exe");
        log::info!(
            "Copying new file {} to {}",
            vector_executable_source.display(),
            vector_executable_dest.display()
        );
        std::fs::copy(&vector_executable_source, &vector_executable_dest)
            .map(|_| ())
            .context(format!(
    ...
    |app_dir| {
        let vector_cli_source = app_dir.join("install\\bin\\vector.exe");
        let vector_cli_dest = app_dir.join("bin\\vector.exe");
        log::info!(
            "Copying new file {} to {}",
            vector_cli_source.display(),
            vector_cli_dest.display()
        );
        std::fs::copy(&vector_cli_source, &vector_cli_dest)
            .map(|_| ())
            .context(format!(
                "Failed to copy new file {} to {}",
                vector_cli_source.display(),
...
pub(crate) const JOBS: [Job; 2] = [
    |_| {
        ...
        if let Ok(config) = std::env::var("VECTOR_AUTO_UPDATE") {
            match config.as_str() {
                ...
                .context("Anyhow!"),
                _ => panic!("Unknown VECTOR_AUTO_UPDATE value: {}", config),
            }
        } else {
            Ok(())
        }
...
pub(crate) fn perform_update(app_dir: &Path, hwnd: Option<isize>) -> Result<()> {
    ...
    let _ = std::process::Command::new(app_dir.join("Vector.exe"))
...
mod test {
    ...
    fn test_perform_update() {
        ...
        // Simulate a timeout
        unsafe { std::env::set_var("VECTOR_AUTO_UPDATE", "err") };
