...
Vector currently maintains two public releases for macOS:

- [Stable](https://vector.dev/download): This is the primary version that people download and use.
- [Preview](https://vector.dev/releases/preview): which receives updates a week ahead of Stable for early adopters.
...
You will need write access to the Vector repository to do this:

- Checkout `main` and ensure your working copy is clean.
- Run `./script/bump-vector-minor-versions` and push the tags
  and branches as instructed.
- Wait for the builds to appear on [the Releases tab on GitHub](https://github.com/vector-editor/vector/releases) (typically takes around 30 minutes)
...
- After the commits are cherry-picked onto the branch, run `./script/trigger-release {preview|stable}`. This will bump the version numbers, create a new release tag, and kick off a release build.
  - This can also be run from the [GitHub Actions UI](https://github.com/vector-editor/vector/actions/workflows/bump_patch_version.yml)
- Wait for the builds to appear on [the Releases tab on GitHub](https://github.com/vector-editor/vector/releases) (typically takes around 30 minutes)
