...
fn main() {
    ...
    if let Ok(output) = Command::new("git").args(["rev-parse", "HEAD"]).output() {
        if output.status.success() {
            ...
            println!("cargo:rustc-env=VECTOR_COMMIT_SHA={git_sha}");
            ...
            if let Ok(build_profile) = std::env::var("PROFILE") {
                if build_profile == "release" {
                    ...
                    println!(
                        "cargo:warning=Info: using '{git_sha}' hash for VECTOR_COMMIT_SHA env var"
                    );
                }
            }
        }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if let Some(explicit_rc_toolkit_path) = std::env::var("VECTOR_RC_TOOLKIT_PATH").ok() {
            res.set_toolkit_path(explicit_rc_toolkit_path.as_str());
        }
        res.set_icon(icon.to_str().unwrap());
        res.set("FileDescription", "Vector");
        res.set("ProductName", "Vector");
