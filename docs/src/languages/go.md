...
Go support is available natively in Vector.
...
Vector sets the following initialization options for inlay hints:
...
to make the language server send back inlay hints when Vector has them enabled in the settings.
