...
impl OpenRequest {
    pub fn parse(urls: Vec<String>) -> Result<Self> {
        ...
        for url in urls {
            if let Some(server_name) = url.strip_prefix("vector-cli://") {
                this.cli_connection = Some(connect_to_cli(server_name)?);
            } else if let Some(action_index) = url.strip_prefix("vector-dock-action://") {
...
pub fn listen_for_cli_connections(opener: OpenListener) -> Result<()> {
    ...
    let sock_path = paths::data_dir().join(format!("vector-{}.sock", *RELEASE_CHANNEL_NAME));
...
mod tests {
    ...
    async fn test_open_workspace_with_directory(cx: &mut TestAppContext) {
        ...
        // Now open a file inside that workspace, but tell Vector to open a new window
        open_workspace_file(
    ...
    async fn test_open_workspace_with_nonexistent_files(cx: &mut TestAppContext) {
        ...
        // but tell Vector to add it to the current workspace
        ...
        // but tell Vector to NOT add it to the current workspace
