...
async fn test_diagnostics(cx: &mut TestAppContext) {
    ...
    let uri = lsp::url_from_file_path(path!("/test/main.rs")).unwrap();
        ...
        lsp_store
            .update_diagnostics(
                ...
                lsp::PublishDiagnosticsParams {
                    uri: lsp::url_from_file_path(path!("/test/consts.rs")).unwrap(),
                    diagnostics: vec![lsp::Diagnostic {
                        range: lsp::Range::new(
    ...
    lsp_store.update(cx, |lsp_store, cx| {
        ...
        lsp_store
            .update_diagnostics(
                server_id_1,
                lsp::PublishDiagnosticsParams {
                    uri: lsp::url_from_file_path(path!("/test/main.js")).unwrap(),
                    diagnostics: vec![lsp::Diagnostic {
                        ...
                        related_information: Some(vec![lsp::DiagnosticRelatedInformation {
                            location: lsp::Location::new(
                                lsp::url_from_file_path(path!("/test/main.js")).unwrap(),
                                lsp::Range::new(
    ...
    lsp_store.update(cx, |lsp_store, cx| {
        ...
        lsp_store
            .update_diagnostics(
                server_id_2,
                lsp::PublishDiagnosticsParams {
                    uri: lsp::url_from_file_path(path!("/test/main.rs")).unwrap(),
                    diagnostics: vec![],
                    version: None,
                },
                &[],
                ...
                lsp_store.update(cx, |lsp_store, cx| {
                    ...
                    lsp_store
                        .update_diagnostics(
                            server_id,
                            lsp::PublishDiagnosticsParams {
                                uri: lsp::url_from_file_path(&path).unwrap_or_else(|_| {
                                    "file:///test/fallback.rs".parse::<lsp::Url>().unwrap()
                                }),
    ...
    cx.update(|_, cx| {
        lsp_store.update(cx, |lsp_store, cx| {
            lsp_store.update_diagnostics(
                LanguageServerId(0),
                lsp::PublishDiagnosticsParams {
                    uri: lsp::url_from_file_path(path!("/root/dir/file.rs")).unwrap(),
                    version: None,
                    diagnostics: vec![lsp::Diagnostic {
                        range,
                        severity: Some(lsp::DiagnosticSeverity::ERROR),
...
async fn test_diagnostics_with_code(cx: &mut TestAppContext) {
    ...
    let cx = &mut VisualTestContext::from_window(*window, cx);
    let workspace = window.root(cx).unwrap();
    let uri = lsp::url_from_file_path(path!("/root/main.js")).unwrap();
    ...
    let related_information = if rng.gen_bool(0.3) {
        ...
        for i in 0..info_count {
            ...
            related_info.push(lsp::DiagnosticRelatedInformation {
                location: lsp::Location::new(lsp::url_from_file_path(path).unwrap(), info_range),
