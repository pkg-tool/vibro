...
Vector is designed to be configured: we want to fit your workflow and preferences exactly. We provide default settings that are designed to be a comfortable starting point for as many people as possible, but we hope you will enjoy tweaking it to make it feel incredible.

In addition to the settings described here, you may also want to change your [theme](./themes.md), configure your [key bindings](./key-bindings.md), set up [tasks](./tasks.md) or install [extensions](./extensions.md).

## Settings files

<!--
TBD: Settings files. Rewrite with "remote settings" in mind (e.g. `local settings` on the remote host).
Consider renaming `vector: Open Local Settings` to `vector: Open Project Settings`.
...
Your settings file can be opened with {#kb vector::OpenSettings}. By default it is located at `~/.config/vector/settings.json`, though if you have XDG_CONFIG_HOME in your environment on Linux it will be at `$XDG_CONFIG_HOME/vector/settings.json` instead.

This configuration is merged with any local configuration inside your projects. You can open the project settings by running {#action vector::OpenProjectSettings} from the command palette. This will create a `.vector` directory containing`.vector/settings.json`.
...
You can find the default settings for your current Vector by running {#action vector::OpenDefaultSettings} from the command palette.
...
```sh
ls ~/Library/Application\ Support/Vector/extensions/installed/
```

On Linux:

```sh
ls ~/.local/share/Vector/extensions/installed
```
...
1. Restore all workspaces that were open when quitting Vector:
...
Vector supports all OpenType features that can be enabled or disabled for a given buffer or terminal font, as well as setting values for font features.
...
- Description: Files or globs of files that will be excluded by Vector entirely. They will be skipped during file scans, file searches, and not be displayed in the project file tree. Overrides `file_scan_inclusions`.
- Default:
...
- Description: Files or globs of files that will be included by Vector, even when ignored by git. This is useful for files that are not tracked by git, but are still important to your project. Note that globs that are overly broad can slow down Vector's file scanning. `file_scan_exclusions` takes precedence over these inclusions.
- Default:
...
- Setting: `file_types`
- Description: Configure how Vector selects a language for a file based on its filename or extension. Supports glob entries.
- Default:

```json
"file_types": {
  "JSONC": ["**/.vector/**/*.json", "**/vector/**/*.json", "**/.vscode/**/*.json"],
  "Shell Script": [".env.*"]
}
```
...
- Description: The icon theme setting can be specified in two forms - either as the name of an icon theme or as an object containing the `mode`, `dark`, and `light` icon themes for files/folders inside Vector.
- Setting: `icon_theme`
...
The following languages have inlay hints preconfigured by Vector:

- [Go](https://docs.vector.dev/languages/go)
- [Rust](https://docs.vector.dev/languages/rust)
- [Svelte](https://docs.vector.dev/languages/svelte)
- [Typescript](https://docs.vector.dev/languages/typescript)

Use the `lsp` section for the server configuration. Examples are provided in the corresponding language documentation.

Hints are not instantly queried in Vector, two kinds of debounces are used, either may be set to 0 to be disabled.
...
- Description: Configure a network proxy for Vector.
- Setting: `proxy`
- Default: `null`
...
By default no proxy will be used, or Vector will attempt to retrieve proxy settings from environment variables, such as `http_proxy`, `HTTP_PROXY`, `https_proxy`, `HTTPS_PROXY`, `all_proxy`, `ALL_PROXY`.
...
- Description: The theme setting can be specified in two forms - either as the name of a theme or as an object containing the `mode`, `dark`, and `light` themes for the Vector UI.
- Setting: `theme`
- Default: `One Dark`
...
- Description: The name of the dark Vector theme to use for the UI.
- Setting: `dark`
- Default: `One Dark`
...
- Description: The name of the light Vector theme to use for the UI.
- Setting: `light`
- Default: `One Light`
...
Vector supports all OpenType features that can be enabled or disabled for a given UI font, as well as setting values for font features.
...
// ~/.config/vector/settings.json
{
  ...
  "buffer_font_size": 18,
  "buffer_font_family": "Vector Plex Mono",
