...
impl ProtoConversion for TcpArgumentsTemplate {
    type ProtoType = proto::TcpHost;
    type Output = Result<Self>;

    fn to_proto(&self) -> Self::ProtoType {
        proto::TcpHost {
            port: self.port.map(|port| port as u32),
            host: self.host.map(|host| host.to_string()),
    ...
    fn from_proto(payload: Self::ProtoType) -> Result<Self> {
        let port = payload
            .port
            .map(|port| u16::try_from(port).context("tcp port out of range"))
            .transpose()?;
        ...
        let host = payload
            ...
            .map(|host| {
                host.parse::<Ipv4Addr>()
                    .with_context(|| format!("invalid tcp host: {host}"))
            })
            .transpose()?;
