...
- `docs-proxy` is used for `https://vector.dev/docs`

On push to `main`, both of these (and the files they depend on) are uploaded to Cloudflare.

### Deployment

These functions are deployed on push to main by the deploy_cloudflare.yml workflow. Worker Rules in Cloudflare intercept requests to vector.dev and proxy them to the appropriate workers.
