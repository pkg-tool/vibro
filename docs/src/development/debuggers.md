...
> **DISCLAIMER**: This is not documentation for the [planned debugger support in Vector](https://github.com/vector-editor/vector/issues/5065).
> Rather, it is intended to provide information on how to use an external debugger while developing Vector itself to both Vector employees and external contributors.
> Once debugger support is implemented, this section will be updated to provide information on how to use the built-in debugger as part of Vector development.
...
Therefore, in order to use a debugger to it's fullest extent, you must compile a new Vector binary, with full debug info.
...
#### Running Vector with a Debugger

After following the steps above for including full debug info when compiling Vector,
You can either run `rust-gdb` or `rust-lldb` on the compiled Vector binary after building it with `cargo build`, by running one of the following commands:
...
To get the process ID of a running Vector instance, you can use your systems process management tools such as `Task Manager` on windows or `Activity Monitor` on MacOS.

Alternatively, you can run the `ps aux | grep vector` command on MacOS and Linux or `Get-Process | Select-Object Id, ProcessName` in an instance of PowerShell on Windows.
