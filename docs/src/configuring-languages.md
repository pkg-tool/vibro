...
- [`tab_size`](./configuring-vector.md#tab-size): The number of spaces for each indentation level
- [`formatter`](./configuring-vector.md#formatter): The tool used for code formatting
- [`format_on_save`](./configuring-vector.md#format-on-save): Whether to automatically format code when saving
- [`enable_language_server`](./configuring-vector.md#enable-language-server): Toggle language server support
- [`hard_tabs`](./configuring-vector.md#hard-tabs): Use tabs instead of spaces for indentation
- [`preferred_line_length`](./configuring-vector.md#preferred-line-length): The recommended maximum line length
- [`soft_wrap`](./configuring-vector.md#soft-wrap): How to wrap long lines of code
- [`show_completions_on_input`](./configuring-vector.md#show-completions-on-input): Whether or not to show completions as you type
- [`show_completion_documentation`](./configuring-vector.md#show-completion-documentation): Whether to display inline and alongside documentation for items in the completions menu
...
2. Storage Location:
   ...
   - macOS: `~/Library/Application Support/Vector/languages`
...
This disables the language server for Markdown files, which can be useful for performance in large documentation projects. You can configure this globally in your `~/.config/vector/settings.json` or inside a `.vector/settings.json` in your project directory.
...
Vector uses Tree-sitter grammars for syntax highlighting. Override the default highlighting using the `experimental.theme_overrides` setting.
