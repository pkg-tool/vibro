...
use gpui::{App, AsyncApp, Context, Entity, EventEmitter, Subscription, Task};
...
impl BreakpointStore {
    ...
    pub fn with_serialized_breakpoints(
        ...
        let mode = match &self.mode {
            BreakpointStoreMode::Local(mode) => mode.clone(),
            #[cfg(feature = "collab")]
            BreakpointStoreMode::Remote(_) => return Task::ready(Ok(())),
        };
...
impl Breakpoint {
    ...
    ) -> Option<proto::Breakpoint> {
        Some(proto::Breakpoint {
    ...
    fn from_proto(breakpoint: proto::Breakpoint) -> Option<Self> {
        Some(Self {
