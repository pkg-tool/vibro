//! # Component Preview
//!
//! A view for exploring Vector components.
    ...
    fn test_status_toast(&self, cx: &mut Context<Self>) {
        if let Some(workspace) = self.workspace.upgrade() {
            workspace.update(cx, |workspace, cx| {
                let status_toast =
                    StatusToast::new("`vector/new-notification-system` created!", cx, |this, _| {
...
impl ComponentPreviewPage {
    pub fn new(component: ComponentMetadata) -> Self {
        Self {
            // languages,
            component,
        }
    ...
    fn render_preview(&self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        v_flex()
            ...
            .child(
                self.component
                    .preview()
                    .and_then(|preview| preview(window, cx))
                    .unwrap_or_else(|| div().child("No preview available").into_any_element()),
            )
    }
}
